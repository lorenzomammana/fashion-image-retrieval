...
class FashionModel():
    ...
    def build(self):
        ...
        encoded = keras.layers.MaxPooling2D(poolkernel, padding='same')(x)

        x = keras.layers.Conv2D(n_hidden_3, convkernel, activation='relu', padding='same')(encoded)
        x = keras.layers.UpSampling2D(poolkernel)(x)
        x = keras.layers.Conv2D(n_hidden_2, convkernel, activation='relu', padding='same')(x)
        x = keras.layers.UpSampling2D(poolkernel)(x)
        x = keras.layers.Conv2D(n_hidden_1, convkernel, activation='relu')(x)
        x = keras.layers.UpSampling2D(poolkernel)(x)
        decoded = keras.layers.Conv2D(self.shape_img[2], convkernel, activation='sigmoid', padding='same')(x)

        classification = keras.layers.Dense(self.n_classes, activation='softmax', name='output_class')(encoded)
    ...
    def compile(self):
        ...
        self.autoencoder.compile(optimizer=self.optimizer, loss=self.losses)
